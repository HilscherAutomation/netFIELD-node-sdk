    ...
    if ((authStrategy === 'auth') && utils.isTokenExpired(config.tokenExpiresAt)) {
        return api.refreshToken()
            .then(function () {
                ...
                return sendFormData(opts.path, config.userToken, formData, requestMethod, callback);
            });
    } else {
        var builtPath = config.schema + '://' + opts.hostname + ':' + opts.port + opts.path;
        return sendFormData(builtPath, config.userToken, formData, requestMethod, callback);
    }
};
        ...
        var req = client.request(httpOptions, function (res) {
            ...
            res.on('end', function () {
                ...
                if (chunks) {
                    ...
                    if (res.statusCode === 401 || (res.statusCode === 403 && res.url.endsWith('auth') && postData.indexOf('refreshToken') >= 0 )) {
                        if (window) {
...
var sendFormData = function sendFormData(url, token, formData, method, callback) {
    var status;
    var requestMethod = method || 'POST';
    return new Promise(function (resolve, reject) {
        var request = {
